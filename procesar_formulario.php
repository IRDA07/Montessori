...
$mensaje = htmlspecialchars(trim($_$_POST['message'] ?? ''));
...
$recaptcha_secret_key = '6LdlBZYrAAAAAIsL1mPMlg0Ha_k0xAnYg93dgS6w';
...
if (empty($recaptcha_response)) {
    ...
    if ($captcha_response && $captcha_response->success === true) {
        $captcha_verified = true;
    } else {
        $errors[] = "Verificación CAPTCHA fallida. Inténtalo de nuevo.";
        error_log("reCAPTCHA verification failed for IP: " . $_SERVER['REMOTE_ADDR'] . " Errors: " . json_encode($captcha_response->{'error-codes'} ?? 'None'));
    }
}
